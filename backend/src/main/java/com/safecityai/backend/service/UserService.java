...
public class UserService {
    ...
    // SEGURIDAD: Siempre responde OK aunque el email no exista,
    // para evitar que un atacante confirme qué emails están registrados.
    @Transactional
    public void forgotPassword(ForgotPasswordDTO dto) {
        var optionalUser = userRepository.findByEmail(dto.getEmail());
        if (optionalUser.isEmpty()) {
            log.info("Solicitud de reset para email no registrado: {}", dto.getEmail());
